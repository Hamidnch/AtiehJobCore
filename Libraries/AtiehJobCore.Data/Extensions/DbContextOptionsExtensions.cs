...
namespace AtiehJobCore.Data.Extensions
{
    public static class DbContextOptionsExtensions
    {
        ...
        /// </summary>
        public static void AddEntityFrameworkByActiveDatabase(
            this IServiceCollection serviceCollection, ActiveDatabase activeDatabase)
        {
        ...
        public static void SetDbContextOptions(
            this DbContextOptionsBuilder optionsBuilder, SiteSettings siteSettings)
        {
            switch (siteSettings.ActiveDatabase)
            {
                case ActiveDatabase.InMemoryDatabase:
                    optionsBuilder.UseInMemoryDatabase(
                        siteSettings.ConnectionStrings.LocalDb.InitialCatalog);
                    break;
            ...
            switch (siteSettingsValue.ActiveDatabase)
            {
                ...
                case ActiveDatabase.LocalDb:
                    ...
                    var attachDbFilename =
                        siteSettingsValue.ConnectionStrings.LocalDb.AttachDbFilename;

                    var webRootPath = Path.Combine(
                        AppContext.BaseDirectory.Split(new[] { "bin" },
                            StringSplitOptions.RemoveEmptyEntries).First(), "wwwroot");

                    var attachDbFilenamePath = Path.Combine(webRootPath, "App_Data", attachDbFilename);
                    var localDbInitialCatalog =
                        siteSettingsValue.ConnectionStrings.LocalDb.InitialCatalog;
                    return $@"Data Source=(LocalDB)\MSSQLLocalDB;Initial Catalog={localDbInitialCatalog};
                            AttachDbFilename={attachDbFilenamePath};
                            Integrated Security=True;MultipleActiveResultSets=True;";

                case ActiveDatabase.SqlServer:
                    return siteSettingsValue.ConnectionStrings.SqlServer.DbContextConnection;
