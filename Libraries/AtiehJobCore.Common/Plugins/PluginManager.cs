using AtiehJobCore.Common.ComponentModel;
    ...
    public static class PluginManager
    {
                ...
                try
                {
                    ...
                    foreach (var dfd in GetDescriptionFilesAndDescriptors(pluginFolder))
                    {
                        ...
                        if (String.IsNullOrWhiteSpace(pluginDescriptor.SystemName))
                            throw new Exception(
                                $"A plugin '{descriptionFile.FullName}' has no system name. Try assigning the plugin a unique name and recompiling.");
                        if (referencedPlugins.Contains(pluginDescriptor))
                            throw new Exception(
                                $"A plugin with '{pluginDescriptor.SystemName}' system name is already defined");
                        ...
                        try
                        {
                            if (descriptionFile.Directory == null)
                                throw new Exception(
                                    $"Directory cannot be resolved for '{descriptionFile.Name}' description file");
        ...
        public static PluginDescriptor FindPlugin(Type typeInAssembly)
        {
            ...
            return ReferencedPlugins?.FirstOrDefault(plugin => plugin.ReferencedAssembly != null
                                                               && plugin.ReferencedAssembly.FullName.Equals(
                                                                   typeInAssembly.GetTypeInfo().Assembly.FullName,
        ...
        private static Assembly PerformFileDeploy(FileInfo plug, ApplicationPartManager applicationPartManager)
        {
            if (plug.Directory == null || plug.Directory.Parent == null)
                throw new InvalidOperationException("The plugin directory for the " + plug.Name + " file exists in a folder outside of the allowed site folder hierarchy");
            ...
            {
                throw new InvalidOperationException($"The plugin directory for the {plug.Name} file exists in a folder outside of the allowed site folder hierarchy - exception because of {shadowCopiedPlug.FullName} - exception: {ex.Message}");
            }
        }
