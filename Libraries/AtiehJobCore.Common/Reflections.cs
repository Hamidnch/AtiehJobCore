...
namespace AtiehJobCore.Common
{
    public class Reflections
    {
        ...
        private readonly IDictionary<Type, PropertyInfoCache> _propertyCache =
            new Dictionary<Type, PropertyInfoCache>();
        ...
        /// <param name="propertyName">the property to get, can be a
        /// property on a nested object (eg. "Child.Name")</param>
        ...
        /// <param name="propertyName">the property to get,
        /// can be a property on a nested object (eg. "Child.Name")</param>
        ...
        public void SetValue(object target, string propertyName, object value)
        {
            if (propertyName.IndexOf(PropertyNameSeparator) > -1)
            {
                ...
                for (var i = 0; i < propertyList.Length - 1; i++)
                {
                    ...
                    target = Construct(GetType(originalTarget.GetType(),
                        currentFullPropertyNameString));
        ...
        /// </summary>
        /// <param name="propertyList">the array containing
        /// the properties in the corect order</param>
        /// <param name="level">the level up to wich to
        /// include the properties in the returned string</param>
        /// <returns>a dot-separated string containing
        /// the properties up to the given level</returns>
        ...
        private PropertyInfo GetPropertyInfo(Type type, string propertyName)
        {
            ...
            if (propertyInfo == null)
            {
                throw new ArgumentException(string.Format(CultureInfo.InvariantCulture,
                    "Unable to find public property named {0} on type {1}",
                    propertyName, type.FullName), propertyName);
        ...
        {
            var propertyInfos = type.GetProperties(BindingFlags.Public |
            ...
            {
                if (info.Name != propertyName) continue;
                ...
                if (distance <= 0 || distance >= bestMatchDistance) continue;
                bestMatch = info;
        ...
        {
            if (baseType.GetTypeInfo().IsInterface) return -1;
        ...
        {
            if (_propertyCache.ContainsKey(type)) return _propertyCache[type];
            lock (_lockObject)
            {
        ...
        /// <summary>
        /// Creates a new object of the given type, provided that the
        /// type has a default (parameterless)
