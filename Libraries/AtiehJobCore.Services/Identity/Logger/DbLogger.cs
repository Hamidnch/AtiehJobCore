...
namespace AtiehJobCore.Services.Identity.Logger
{
    public class DbLogger : ILogger
    {
            ...
            Func<TState, Exception, string> formatter)
        {
            ...
            var logItem = new LogItem
            {
                Url = httpContextAccessor?.HttpContext != null ?
                    httpContextAccessor.HttpContext.Request.Path.ToString() : string.Empty,
                EventId = eventId.Id,
        ...
        private static void SetStateJson<TState>(TState state, LogItem appLogItem)
        {
            try
            {
        ...
        private void SaveLogItem(LogItem logItem)
        {
            try
            {
                ...
                {
                    context.Set<LogItem>().Add(logItem);
                    context.SaveChanges();
                });
            }
            catch
