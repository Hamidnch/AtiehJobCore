...
namespace AtiehJobCore.Web.Framework.ValidationAttributes
{
    /// <inheritdoc />
    /// <summary>
                        ...
                        case FormValueRequirement.Equal:
                            {
                                ...
                                {
                                    //"name" only
                                    if (routeContext.HttpContext.Request.Form.Keys
                                       .Any(x => x.Equals(buttonName, StringComparison.OrdinalIgnoreCase)))
                                        return true;
                                }
                                else
                                {
                        ...
                        case FormValueRequirement.StartsWith:
                            {
                                if (_validateNameOnly)
                                {
                                    //"name" only
                                    if (routeContext.HttpContext.Request.Form.Keys
                                       .Any(x => x.StartsWith(buttonName, StringComparison.OrdinalIgnoreCase)))
                                        return true;
                                }
                                else
                                {
                                    //validate "value"
                                    if ((from formValue in routeContext.HttpContext.Request.Form.Keys
                                         where formValue.StartsWith(buttonName, StringComparison.OrdinalIgnoreCase)
                                         select routeContext.HttpContext.Request.Form[formValue])
